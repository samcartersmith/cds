...
<VStack alignItems="flex-start" gap={3}>
  {['body', 'caption', 'label1', 'label2', 'legal', 'title1', 'title2', 'title3'].map((font) => (
    <Link font={font} to="https://www.google.com/" openInNewWindow>
      {`${font} link`}
    </Link>
  ))}
</VStack>
...
```jsx
<VStack alignItems="flex-start" gap={3}>
  <Link font="title1" to="https://www.google.com/" openInNewWindow>
    Open window in existing tab
  </Link>
  <Link font="title2" to="https://www.google.com/" rel="noreferrer">
    Sets rel to noreferrer
  </Link>
  <Link font="title3" to="https://www.google.com/" rel="noopener">

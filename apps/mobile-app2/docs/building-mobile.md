...
- If there's any dependency change in `apps/mobile-app2/package.json` and `packages/mobile/package.json`

### How do I rebuild a debug build?

| Platform | Profile - engine type | Command                                       |
| -------- | --------------------- | --------------------------------------------- |
| ios      | local - hermes        | `yarn nx run mobile-app2:build:ios-debug`     |
| android  | local -hermes         | `yarn nx run mobile-app2:build:android-debug` |
...
- Any dependency change in `apps/mobile-app2/package.json` and `packages/mobile/package.json`
...
```shell
yarn nx run mobile-app2:build:ios-release
yarn nx run mobile-app2:build:android-release
```
...
You can create other build types using [app.config.js](/apps/mobile-app2/app.config.ts) and [project.json](/apps/mobile-app2/project.json).

Create a new config in [project.json](/apps/mobile-app2/project.json) `targets.build.configurations`. The key will be come your new command for `yarn nx run mobile-app2:build:<your key>`.

Pass ENVs to configure your build. See [setEnvVars](/apps/mobile-app2/scripts/utils/setEnvVars.mjs) for options and [project.json](/apps/mobile-app2/project.json) for examples.
...
2. Run `yarn nx run mobile-app2:go` to start the development server. This will output a QR code in your terminal.

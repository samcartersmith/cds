...
Get the eas version in [eas.json](/apps/mobile-app2/eas.json) at `cli.version`.
...
**Note: you should only need to build if you're missing your desired build in [prebuilds](/apps/mobile-app2/prebuilds) or if there's been a recent native dependency upgrade.**

See more info about mobile builds [here](/apps/mobile-app2/docs/building-mobile.md).

| Platform | Profile - engine type | Command                                         |
| -------- | --------------------- | ----------------------------------------------- |
| ios      | debug - hermes        | `yarn nx run mobile-app2:build:ios-debug`       |
| ios      | release - hermes      | `yarn nx run mobile-app2:build:ios-release`     |
| android  | debug - hermes        | `yarn nx run mobile-app2:build:android-debug`   |
| android  | release - hermes      | `yarn nx run mobile-app2:build:android-release` |

**Note: If you run into errors when trying to prebuild, check out our [Help](/apps/mobile-app2/docs/help.md) page to debug.**

2. Install app in your simulator with `launch` configuration.

**Note: You can skip this if you've already launched the build in your [prebuilds](/apps/mobile-app2/prebuilds) in your simulator.**

| Platform | Profile - engine type | Command                                          |
| -------- | --------------------- | ------------------------------------------------ |
| ios      | debug - hermes        | `yarn nx run mobile-app2:launch:ios-debug`       |
| ios      | release - hermes      | `yarn nx run mobile-app2:launch:ios-release`     |
| android  | debug - hermes        | `yarn nx run mobile-app2:launch:android-debug`   |
| android  | release - hermes      | `yarn nx run mobile-app2:launch:android-release` |
...
| Platform | Profile - engine type | Command                                       |
| -------- | --------------------- | --------------------------------------------- |
| ios      | debug - hermes        | `yarn nx run mobile-app2:start:ios-debug`     |
| android  | debug - hermes        | `yarn nx run mobile-app2:start:android-debug` |

**Note: If you see `CommandError: No development build (com.ui-systems.debug-ios-hermes) for this project is installed. Please make and install a development build on the device first.` run `yarn clean-expo` and rerun the `start` script. See more debug gotchas [here](/apps/mobile-app2/docs/help.md)**
...
| Platform | Profile - engine type | Command                                         |
| -------- | --------------------- | ----------------------------------------------- |
| ios      | debug - hermes        | `yarn nx run mobile-app2:detox:ios-debug`       |
| ios      | release - hermes      | `yarn nx run mobile-app2:detox:ios-release`     |
| android  | debug -hermes         | `yarn nx run mobile-app2:detox:android-debug`   |
| android  | release -hermes       | `yarn nx run mobile-app2:detox:android-release` |

## An overview of Expo NX Targets

One should note that there are four NX targets associated with Expo that we leverage to build and run mobile-app2 in various contexts. The various contexts can be summarized as a debug and release mode for development and a debug and release mode for visreg tests (powered by detox).

The four NX Targets (also declared in `/apps/mobile-app2/project.json`):
...
These targets call node scripts that live in the [scripts directory of mobile-app2](/apps/mobile-app2/scripts/). These scripts are intuitively named the same as their respective nx targets.

## Expo Debug vs Release Builds

There are eight relevant build variations associated with mobile-app2:
...
Run iOS visreg debug tests: `yarn nx run mobile-app2:detox:ios-debug`
Run iOS visreg release tests: `yarn nx run mobile-app2:detox:ios-release`

## How visreg patching works

The key to understand Visreg tests is by investigating the `apps/mobile-app2/scripts/detox.mjs` script.

...
```shell
yarn workspace mobile-app2 add expo@^<version on npm>
```
...
```shell
cd apps/mobile-app2 && npx expo install --fix
```
...
**This is super important because that native versions must match for the mobile-app2 build to be successful**

4. Upgrade @cbhq/metro-config if applicable. Sometimes metro changes are necessary to resolve react native bump errors. App Infra will update @cbhq/metro-config and expo handles the default expo configurations automatically.

5. Nuke your repo. Cached versions will be compiled in the expo build step and lead to version mismatches. .nx, apps/mobile-app2/expo, apps/mobile-app2/ios, apps/mobile-app2/android should all be removed. Node Modules should be removed because of version mismatches as well. Start fresh :)
...
```shell
yarn nx run mobile-app2:build:ios-debug
yarn nx run mobile-app2:build:android-debug
...
[See our help docs](/apps/mobile-app2/docs/help.md)

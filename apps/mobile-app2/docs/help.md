...
1. How do I run `gradlew` locally for Android debugging?
   Our expo builds are 'managed', and therefore are built in a temp directory outside of our repo that is cleaned at the end of an `eas` command. This means we do not keep `/ios` or `/android` directories in our `mobile-app2`.

To generate a local android directory to run `gradle`:

- `yarn workspace mobile-app2 expo prebuild --platform android`
- `cd apps/mobile-app2/android`
- Run gradle command. You can always find the gradle commands executed for debug and release builds in [eas.json](/apps/mobile-app2/eas.json). For debug, you can run `./gradlew :app:assembleDebug :app:assembleAndroidTest -DtestBuildType=debug`. For release, you can run `./gradlew :app:assembleRelease :app:assembleAndroidTest -DtestBuildType=release`
...
- `yarn workspace mobile-app2 expo prebuild --platform ios`
...
Our expo builds are 'managed', and therefore are built in a temp directory outside of our repo that is cleaned at the end of an `eas` command. In order to see the build output or logs generated from the `yarn nx run mobile-app2:build:<config>`, you need to skip expo cleanup. This can also be used for `launch`

- Go to our [build script](/apps/mobile-app2/scripts/build.mjs)
...
1. `yarn nx run mobile-app2:build:ios-debug` is throwing a `Error: spawn pod ENOENT` error.

- Run `yarn workspace mobile-app2 run expo prebuild -p ios --clean`
- `yarn clean-expo`
- `yarn nx run mobile-app2:build:ios-debug` should work as expected

2. `yarn nx run mobile-app2:build:android-debug` is throwing this error `mobile-app2/android directory not found`

- Run `mkdir apps/mobile-app2/android`
- `yarn nx run mobile-app2:build:android-debug` should work as expected
- Delete the `mobile-app2/android` directory

3. An error like "You are on eas-cli@3.7.2 which does not satisfy the CLI version constraint in eas.json (3.8.1)"

Look up the `cli.version` in `apps/mobile-app2/eas.json`.
...
This error occurs because mobile-app2/ios directory was present at the time of launching the build onto a simulator. This interferes with expos naming of the app on the device.
...
- The packages are incorrectly built. All packages should be built in the preinstall `eas-build-pre-install` script found in the `mobile-app2/package.json` file. If the package has an empty or missing `lib` in the expo temp directory, it didn't correctly build.
...
- Run `yarn nx run mobile-app2:start` multiple times to confirm you are no longer seeing the warning

10. Expo is throwing this error while I'm running a build or launch command (`yarn nx run mobile-app2:<build|launch>:<build type>`): Props Authentication Token not found, Props token or EXPO login error.
...
```shell
cd apps/mobile-app2 && eas build --local --non-interactive --json --clear-cache --platform ios --profile debug
```

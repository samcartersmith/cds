...
if (profile === 'debug') {
  within(async () => {
    await $`cd ../../ && yarn nx run mobile-app2:launch --profile ${profile} --jsEngine ${jsEngine} --platform ${platform}`;
    await $`cd ../../ && yarn nx run mobile-app2:start --profile ${profile} --jsEngine ${jsEngine} --platform ${platform}`;
  });
}
...
if (platform === 'ios') {
  await $`yarn workspace mobile-app2 detox rebuild-framework-cache`;
}

// Clear Jest cache
await $`yarn workspace mobile-app2 detox test --configuration ${platform}-${profile} --clearCache`;

const platformOptions = platform === 'android' ? '--force-adb-install' : '';

if (isCI) {
  await $`yarn workspace mobile-app2 detox test --configuration ${platform}-${profile} --headless --cleanup ${platformOptions}`;
} else {
  await $`yarn workspace mobile-app2 detox test --loglevel verbose --configuration ${platform}-${profile} --cleanup ${platformOptions}`;
}

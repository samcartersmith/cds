 ...
 *
 * Until we update our docs directory structure to match our sidebar structure, we will have to explicitly
 * define each sidebar item in order for it to appear on the website. This will not happen automatically.
 */
 ...
 * - Other categories may use the autogenerated docusaurus feature. For those use cases, you can add
 * `draft: true` to the frontmatter section of the doc you want to hide in production.
 */
...
/* -------------------------------------------------------------------------- */
/**
 * Most component doc's are at the root of the  "Components" category, however,
 * there are some components where it makes more sense to bucket it under a family or some base component.
 *
 * For example, RemoteImageGroup is a child of the RemoteImage category.
 *
 ...
 *
 * You should not only work with your design partner to align on naming/api proposals, but also where it will
 * live on the website. It's often during these conversations that you might reconsider the component name entirely.
 *
 */
...
/**
 *
 * Within each category they follow varying degrees of sorting, but if we were to
 * define some logic it would be:
 ...
 *    - For example,
 *
 * Each added item or category  should be alphabetized unless the doc is somewhat
 * supplementary info.
 *
...
/* -------------------------------------------------------------------------- */
/**
 * The categories we currently have were purposfally organized in this way and we want
 * to be more purposeful of any changes to this organization moving forward.
 *

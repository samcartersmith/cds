...
function Example() {
  ...
  return (
    <VStack spacing={4}>
      <VStack minHeight={450} width="100%" spacingTop={5} gap={3}>
        <HStack gap={2}>
          <Button compact onPress={handleOpenButtonPress}>
            Open
          </Button>
          <Button compact onPress={handleCloseButtonPress}>
            Close
          </Button>
        </HStack>
        <Dropdown value={value} onChange={setValue} content={content} ref={dropdownRef}>
          <IconButton name="more" />
        </Dropdown>
      </VStack>
    </VStack>

...
<Tabs groupId="platform" variant="secondary">
  <TabItem value="web" label="Web">
    <>
      <TextTitle4 as="h3">Getting started</TextTitle4>
      <TextBody as="span">No additional peerDependencies are required for web</TextBody>
    </>
  </TabItem>
  <TabItem value="mobile" label="Mobile">
    <>
      <TextTitle4 as="h3">Getting started</TextTitle4>
      <TextBody as="span">
        Install <code>react-native-date-picker</code> for mobile
      </TextBody>
      <CodeBlock language="bash">{`yarn add react-native-date-picker@${mobilePackage.peerDependencies['react-native-date-picker']}`}</CodeBlock>
    </>
  </TabItem>
</Tabs>
&nbsp;
...
The date format is automatically adjusted to the `LocaleContext`. See the [LocaleProvider docs here](/components/locale-provider).

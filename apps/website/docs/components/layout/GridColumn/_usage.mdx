GridColumns can be used to explicitly place children of a [Grid](/components/Grid) between specified grid lines (the vertical and horizontal lines that make up a Grid).
  ...
  return (
    <Grid gap={0.5} columns={12}>
      {Array.from({ length: 12 }).map((_, idx) => (
        <GridColumn background="primary" spacing={2} colStart={1} colEnd={idx + 2} key={idx} />
      ))}
    </Grid>
  );
}
...
function FullBleedExample() {
  ...
  return (
    <Grid gap={0.5} templateColumns="100px 1fr 100px">
      ...
      {/* alternatively, you could also use colSpan={3} if you know how many columns there are */}
        gridColumn="1 / -1"
        justifyContent="center"
        background="backgroundAlternate"
        spacing={2}

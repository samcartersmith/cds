...
Like any component system, much of responsibility for building accessible UI's is in your hands as the consumer to properly implement the component composition. We'll do our best to provide sane fallbacks, but here's the biggest gotcha for `TextInput`'s you can watch out for'.
...
It's also advised you always format `helperText` with `Error: ${erroMessage}`. We'd do that for you, but _i18n_ isn't baked into CDS. Take a look at the example below:

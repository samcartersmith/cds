...
describe('transformers', () => {
  ...
  describe('transformActionsCumulative', () => {
    ...
    it('should sum insertions and detachments across all teams and weeks into exactly 2 metrics', () => {
      const { metrics, draftsOmitted } = transformActionsCumulative(
        multiWeekTeamData,
        mockLibraryFileKey,
        mockLibraryName,
        'mtd',
      ...
      // Always exactly 2 metrics: one insertion, one detachment
      expect(metrics).toHaveLength(2);
      // Drafts row is excluded
      expect(draftsOmitted).toBe(1);

      const insertionMetric = metrics.find((m) => m.tags.action_type === 'insertion');
      const detachmentMetric = metrics.find((m) => m.tags.action_type === 'detachment');

      // Sum across both teams and both weeks, excluding Drafts:
      // insertions: 20 + 22 + 10 + 18 = 70
      // detachments: 2 + 3 + 1 + 2 = 8
      expect(insertionMetric?.value).toBe(70);
      expect(detachmentMetric?.value).toBe(8);
    });

    it('should use the figma_lib.actions.cumulative metric name', () => {
      const { metrics } = transformActionsCumulative(
        multiWeekTeamData,
        mockLibraryFileKey,
        mockLibraryName,
        'mtd',
      ...
      expect(metrics.every((m) => m.metricName === 'figma_lib.actions.cumulative')).toBe(true);
    });
    ...
    it('should not include team or component tags', () => {
      const { metrics } = transformActionsCumulative(
        multiWeekTeamData,
        mockLibraryFileKey,
        mockLibraryName,
      ...
      expect(metrics.every((m) => !('figma_team' in m.tags))).toBe(true);
      expect(metrics.every((m) => !('component_name' in m.tags))).toBe(true);
    });
    ...
    it('should filter out draft entries', () => {
      ...
      expect(draftsOmitted).toBe(1);
      // Drafts insertions (5) and detachments (1) must not appear in the totals
      const insertionMetric = metrics.find((m) => m.tags.action_type === 'insertion');
      expect(insertionMetric?.value).toBe(70); // not 75
    });

    it('should return 2 zero-value metrics for empty input', () => {
      const { metrics, draftsOmitted } = transformActionsCumulative(
        [],
        mockLibraryFileKey,
        mockLibraryName,
    ...
    it('should only count rows within the period when pre-filtered by the caller', () => {
      // Rows spanning Q3 and Q4 2026. main.ts filters to rows whose `week` >= quarter start
      // before passing them to this transformer, so only the Q4 rows are included.
      const crossQuarterData: LibraryAnalyticsComponentActionsByTeam[] = [
        // Q3 2026 rows — excluded by pre-filter
        {
          team_name: 'Design Team',
          workspace_name: 'Main Workspace',
          week: '2026-07-05',
      ...
      // Only Q4 weeks are counted: 20 + 15 = 35 insertions, 2 + 1 = 3 detachments
      expect(insertionMetric?.value).toBe(35);
      expect(detachmentMetric?.value).toBe(3);
    });
  });
});

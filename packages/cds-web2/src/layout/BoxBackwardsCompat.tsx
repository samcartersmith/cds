...
import { type StaticStyleProps, type StyleProps, getStyles } from '../styles/styleProps';
...
type BoxBackwardsCompatProps = {
  spacing?: StyleProps['padding'];
  spacingTop?: StyleProps['paddingTop'];
  spacingBottom?: StyleProps['paddingBottom'];
  spacingStart?: StyleProps['paddingLeft'];
  spacingEnd?: StyleProps['paddingRight'];
  spacingHorizontal?: StyleProps['paddingX'];
  spacingVertical?: StyleProps['paddingY'];
  offset?: StyleProps['padding'];
  offsetTop?: StyleProps['paddingTop'];
  offsetBottom?: StyleProps['paddingBottom'];
  offsetStart?: StyleProps['paddingLeft'];
  offsetEnd?: StyleProps['paddingRight'];
  offsetHorizontal?: StyleProps['paddingX'];
  offsetVertical?: StyleProps['paddingY'];
};

export type BoxBaseProps = StyleProps &
  BoxBackwardsCompatProps &
  SharedProps &
  Pick<
...
const convertOffsetToMargin = (value?: StyleProps['paddingTop']): StyleProps['marginTop'] => {
  if (!value) return value;
  if (typeof value === 'number') return (value * -1) as StaticStyleProps['marginTop'];
  const result: StyleProps['marginTop'] = {};
  // eslint-disable-next-line guard-for-in
  for (const key in value) {
    const responsiveValue = value[key as keyof typeof value];
    if (typeof responsiveValue === 'number')
      result[key as keyof typeof value] = (responsiveValue * -1) as StaticStyleProps['marginTop'];
  }
};
  ...
  <AsComponent extends React.ElementType>(
    {
      ...
      gridArea,
      opacity,
      // Begin backwards compatible style props
      spacing,
      spacingTop,
      spacingBottom,
      spacingStart,
        ...
        getStyles(
          {
            ...
            paddingX: spacingHorizontal ?? paddingX,
            paddingY: spacingVertical ?? paddingY,
            paddingTop: spacingTop ?? paddingTop,
            paddingBottom: spacingBottom ?? paddingBottom,
            paddingLeft: spacingStart ?? paddingLeft,
            paddingRight: spacingEnd ?? paddingRight,
            margin: convertOffsetToMargin(offset) ?? margin,
            marginX: convertOffsetToMargin(offsetHorizontal) ?? marginX,
            marginY: convertOffsetToMargin(offsetVertical) ?? marginY,
            marginTop: convertOffsetToMargin(offsetTop) ?? marginTop,
            marginBottom: convertOffsetToMargin(offsetBottom) ?? marginBottom,
            marginLeft: convertOffsetToMargin(offsetStart) ?? marginLeft,
            marginRight: convertOffsetToMargin(offsetEnd) ?? marginRight,
            userSelect,
            visibility,
            width,
            height,

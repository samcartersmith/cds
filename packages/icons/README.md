...
8. Locate your commit in [Codeflow](https://codeflow.cbhq.net/#/frontend/cds/commits) and check that `package-cds-icons` has been built, and that the following targets have been deployed once the entire build is finished:

- `corporate::cds-icons`
- `production::cds-docs`

If not, manually trigger the build if necessary and/or deploy the targets as needed when the build is complete. **_NOTE:_** If you're releasing both icons and illustrations at the same time, you only need to deploy to `production::cds-docs` once, so just pick whichever commit is the most recent and deploy from there.

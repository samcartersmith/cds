...
export type TourStepArrowComponent = React.ForwardRefExoticComponent<
  ...
  TourStepArrowComponentProps & React.RefAttributes<any>
>;
...
export type TourOptions = {
  ...
  activeTourStep: TourStepValue | null;
  /* Set the value of the currently active tour step. */
  onChange: (tourStep: TourStepValue | null) => void;
};

export type TourApi = Omit<TourOptions, 'onChange'> & {
  /* Jumps to a specified step of the tour. */
  setActiveTourStep: (tourStepId: string | null) => void;
  /* Starts the tour; can optionally start at a specified step ID. */
  startTour: (tourStepId?: string) => void;
  /* Stops the tour. */
  stopTour: () => void;
...
export const useTour = ({ steps, activeTourStep, onChange }: TourOptions): TourApi => {
  const startTour = useCallback(
    async (tourStepId?: string | null) => {
  ...
  const setActiveTourStep = useCallback(
    async (tourStepId: string | null) => startTour(tourStepId),
    [startTour],
  );

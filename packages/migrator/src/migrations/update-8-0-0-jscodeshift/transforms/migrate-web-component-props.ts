/**
 * Codemod to transform specific web component props for CDS web components.
 *
...
const componentPropMapping = Object.fromEntries([
  ...
  typeof textPropMapping | typeof tooltipPropMapping | typeof cellPropMapping
>;
...
export default function transformer(file: FileInfo, api: API, options: Options) {
  ...
  // 1. Find all imported component names from @cbhq/cds-web*
  ...
  elements.forEach((path: ASTPath<JSXElement>) => {
    ...
    // For component variables, use text component prop mapping since they resolve to text components
    let propMapping;
    if (componentVariableNames.has(componentName)) {
      propMapping = textPropMapping;

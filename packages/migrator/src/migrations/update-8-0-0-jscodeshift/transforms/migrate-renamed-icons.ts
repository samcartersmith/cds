...
export default function transformer(file: FileInfo, api: API, options: Options) {
  ...
  root.find(j.ImportDeclaration).forEach((path: ASTPath<ImportDeclaration>) => {
    ...
    if (
      ...
      (sourceValue.startsWith(targetPackage) || sourceValue.startsWith(':rn/cds-wallet/components')) // TODO remove this once wallet is migrated
    ) {
      path.value.specifiers?.forEach((spec) => {
        if (spec.type === 'ImportSpecifier') {
          ...
          if (Object.keys(componentPropMap).includes(importedName)) {
  ...
  root.find(j.JSXElement).forEach((path) => {
    ...
    if (openingElement.name.type === 'JSXIdentifier') {
      ...
      if (importedComponents.has(componentName) && componentPropMap[componentName]) {

...
export const commonPathMigrationMap: Record<string, string> = {
  IconName: '@cbhq/cds-common',
  HeroSquareName: '@cbhq/cds-common',
  SpotRectangleName: '@cbhq/cds-common',
  SpotSquareName: '@cbhq/cds-common',
  PictogramName: '@cbhq/cds-common',
  SpotIconName: '@cbhq/cds-common',
  ...
  IllustrationHeroSquareNames: '@cbhq/cds-common',
  ...
  FlexStyles: '@cbhq/cds-common',
  CellSpacingConfig: '@cbhq/cds-common',
  IconSize: '@cbhq/cds-common',
  AvatarSize: '@cbhq/cds-common',
  LottieSource: '@cbhq/cds-common',
};
...
// Define the mapping for mobile-only imports
...
export default function transformer(file: FileInfo, api: API, options: Options) {
  ...
  let pathMigrationMap = getPathMigrationMap(platform);

  // If a typeName is specified, filter the map to only that type
  if (typeName) {
    const newPath = pathMigrationMap[typeName];
    if (newPath) {
      pathMigrationMap = { [typeName]: newPath };
    } else {
      // If the provided typeName is not in the migration map for the platform, do nothing.
      return file.source;
    }
  }
    ...
    if (targetedImports.length > 0) {
      ...
      importedNames.forEach((name) => {
        if (Object.prototype.hasOwnProperty.call(pathMigrationMap, name)) {
          ...
          if (currentBasePackage === targetBasePackage && !currentSourcePath.includes('/v7')) {

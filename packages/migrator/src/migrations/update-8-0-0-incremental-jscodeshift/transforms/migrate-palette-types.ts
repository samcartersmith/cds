...
export const PALETTE_TYPES_TO_MIGRATE = {
...
export default function transformer(file: FileInfo, api: API, options: Options) {
  ...
  let activePaletteTypesToMigrate: { [key: string]: string } = PALETTE_TYPES_TO_MIGRATE;

  if (typeName) {
    if (Object.prototype.hasOwnProperty.call(PALETTE_TYPES_TO_MIGRATE, typeName)) {
      activePaletteTypesToMigrate = {
        [typeName]: PALETTE_TYPES_TO_MIGRATE[typeName as keyof typeof PALETTE_TYPES_TO_MIGRATE],
      };
    } else {
  ...
  function createTypeReference(oldTypeName: string) {
    const newTypeName =
      activePaletteTypesToMigrate[oldTypeName as keyof typeof activePaletteTypesToMigrate];
    ...
    .forEach((importPath) => {
      importPath.node.specifiers?.forEach((specifier) => {
        if (specifier.type === 'ImportSpecifier') {
          ...
          if (Object.keys(activePaletteTypesToMigrate).includes(importedName)) {
  ...
  const needsThemeVarsImport = paletteImports.some(({ originalName }) =>
    activePaletteTypesToMigrate[originalName].startsWith('ThemeVars.'),
  );
  const needsColorSchemeImport = paletteImports.some(
    ({ originalName }) => activePaletteTypesToMigrate[originalName] === 'ColorScheme',
  );
  const needsThemeImport = paletteImports.some(
    ({ originalName }) => activePaletteTypesToMigrate[originalName] === 'Partial<Theme["color"]>',
  );

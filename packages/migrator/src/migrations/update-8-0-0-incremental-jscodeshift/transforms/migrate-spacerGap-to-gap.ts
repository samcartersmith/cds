...
export default function transformer(file: FileInfo, api: API, options: Options) {
  ...
  const elements = root.find(j.JSXElement).filter((path: ASTPath<JSXElement>) => {
    ...
    if (openingElement.name.type === 'JSXIdentifier') {
      ...
      if (targetComponent) {
        return componentName === targetComponent && relevantTagNames.has(componentName);
      }

      return relevantTagNames.has(componentName);
    }
    if (openingElement.name.type === 'JSXMemberExpression') {
      ...
      if (object.type === 'JSXIdentifier') {
        ...
        if (property.type === 'JSXIdentifier') {
          ...
          // Filter by target component if specified
          if (targetComponent) {
            return componentName === targetComponent && componentNamespaceNames.has(namespaceName);
          }

          return componentNamespaceNames.has(namespaceName);
        }
      }
    }
    return false;

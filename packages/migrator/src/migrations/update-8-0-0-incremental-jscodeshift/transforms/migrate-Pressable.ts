      ...
      path.node.specifiers?.forEach((spec) => {
        if (spec.type === 'ImportSpecifier' && spec.imported.type === 'Identifier') {
          if (spec.imported.name === targetName) {
            ...
            const shouldInclude = explicitTarget ? localName === explicitTarget : true;
            if (shouldInclude) {
...
export default function transformer(file: FileInfo, api: API, options: Options) {
  ...
  if (targetComponent) {
    const requestedFound =
      (targetComponent === PRESSABLE_COMPONENT_NAME &&
        (pressableRefs.relevantTagNames.size > 0 ||
          pressableRefs.componentNamespaceNames.size > 0)) ||
      (targetComponent === PRESSABLE_OPACITY_COMPONENT_NAME &&
        (pressableOpacityRefs.relevantTagNames.size > 0 ||
          pressableOpacityRefs.componentNamespaceNames.size > 0));
  ...
  root
    ...
    .filter((path: ASTPath<JSXElement>) => {
      ...
      if (openingElement.name.type === 'JSXIdentifier') {
        ...
        if (targetComponent) {
          return componentName === targetComponent && allRelevantTagNames.has(componentName);
        }
      ...
      if (openingElement.name.type === 'JSXMemberExpression') {
        ...
        if (
          ...
          if (targetComponent) {
            return property.name === targetComponent;
          }
          return true;
        }
      }

 ...
 * @param targetComponent - Optional specific component to filter by.
...
export default function transformer(file: FileInfo, api: API, options: Options) {
  ...
  if (targetComponent && !ALL_TARGETED_COMPONENTS.includes(targetComponent)) {
  ...
  root
    ...
    .filter((path: ASTPath<JSXElement>) => {
      ...
      if (openingElement.name.type === 'JSXIdentifier') {
        ...
        if (targetComponent) {
          return (
            componentName === targetComponent && combinedAsARelevantTagNames.has(componentName)
          );
        }

        return combinedAsARelevantTagNames.has(componentName);
      }
  ...
  root
    ...
    .filter((path: ASTPath<JSXElement>) => {
      ...
      if (openingElement.name.type === 'JSXIdentifier') {
        ...
        // Filter by target component if specified
        if (targetComponent) {
          return componentName === targetComponent && linkRefs.relevantTagNames.has(componentName);
        }

        return linkRefs.relevantTagNames.has(componentName);
      }
      if (openingElement.name.type === 'JSXMemberExpression') {
        const object = openingElement.name.object;
        const property = openingElement.name.property;

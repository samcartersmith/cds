...
export const typesWithNewPathOrRenamed: Record<
  string,
  { name?: string; path?: string; oldPath?: string[] }
> = {
  ToastRefBaseProps: {
    name: 'ToastRefHandle',
  ...
  ColorScheme: {
    oldPath: ['@cbhq/cds-common', '@cbhq/cds-common/types', '@cbhq/cds-common/types/TagBaseProps'],
    name: 'TagColorScheme',
    path: '@cbhq/cds-common',
  },
  StackBaseProps: {
...
export default function transformer(file: FileInfo, api: API, options: Options) {
  ...
  const typeName = options.typeName as string | undefined;

  let activeTypesToMigrate = typesWithNewPathOrRenamed;

  if (typeName) {
    if (Object.prototype.hasOwnProperty.call(typesWithNewPathOrRenamed, typeName)) {
      activeTypesToMigrate = {
        [typeName]: typesWithNewPathOrRenamed[typeName],
      };
    } else {
      // If the provided typeName is not a migratable type, do nothing.
      return file.source;
    }
  }
  ...
  root
    ...
    .forEach((importPath: ASTPath<ImportDeclaration>) => {
      ...
      importPath.value.specifiers?.forEach((spec) => {
        ...
        if (Object.prototype.hasOwnProperty.call(activeTypesToMigrate, originalImportedName)) {
          const config = activeTypesToMigrate[originalImportedName];
          const currentImportSource = importPath.value.source.value as string;

          // If oldPath is provided, the current import source must match one of the paths in oldPath
          if (config.oldPath && !config.oldPath.includes(currentImportSource)) {
            specifiersToKeep.push(spec); // Keep the specifier as is
            return;
          }

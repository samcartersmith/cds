...
export default function transformer(file: FileInfo, api: API, options: Options) {
  ...
  if (componentName) {
    console.log(`INFO [${file.path}]: Filtering base props for component: '${componentName}'`);
    const filteredMap: Record<string, string> = {};
    for (const propName in basePropToComponentPathMap) {
      // Use exact match for the component name followed by BaseProps
      if (propName === `${componentName}BaseProps`) {
        filteredMap[propName] = basePropToComponentPathMap[propName];
      }
    }
    targetMap = filteredMap;
  }

  // Create a set of just the base prop names
  const basePropNamesSet = new Set(Object.keys(targetMap));

  if (basePropNamesSet.size === 0 && componentName) {
    // Only exit early if a component was specified and nothing matched
    return file.source;
  }
      ...
      generalImportPath.value.specifiers?.forEach((specifier) => {
        if (specifier.type === 'ImportSpecifier' && specifier.imported.type === 'Identifier') {
          ...
          if (basePropNamesSet.has(importedName)) {
            const componentPath = targetMap[importedName];
            if (componentPath) {
              let newPath = '';

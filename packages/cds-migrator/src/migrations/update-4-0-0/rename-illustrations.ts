...
const packageNameToLog = '@cbhq/cds-illustrations';
...
function migrateAttributeValues(args: TransformFnType) {
  ...
  if (
    !oldImportCheck(sourceContent) ||
    !fileIncludesRenamedValue(sourceContent, illustrationRenames)
  ) {
    return;
  }
  ...
  parseJsxElements({
    ...args,
    callback: (jsx) => {
      components.push(getComponentName({ renameMap: illustrationRenames, jsx }));
    },
  });

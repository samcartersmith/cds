...
```shell
yarn mono-pipeline version ui-mobile-playground2
```
...
4. Locate your commit in [Codeflow](https://codeflow.cbhq.net/#/frontend/cds/commits), confirm `package-ui-mobile-playground2` has built, and deploy to `corporate::ui-mobile-playground2` when the build is complete

5. After the deploy has succeeded, verify that the new package was published at the [production Coinbase NPM registry](https://artifactory.cbhq.net/ui/repos/tree/General/cb-npm-master). It usually takes about 10 min or so for the package to be uploaded. Look for the version number at the bottom of the artifact list in the [package directory](https://artifactory.cbhq.net/ui/repos/tree/General/cb-npm-master/@cbhq/ui-mobile-playground2/-/@cbhq/ui-mobile-playground2-1.0.0-rc.1.tgz).

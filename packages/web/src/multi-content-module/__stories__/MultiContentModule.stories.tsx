import { IconName, useSpectrum, useToggler } from '@cbhq/cds-common';
...
const socialMediaItems: SocialMediaItem[] = [
  ...
  {
    name: 'Wallet',
    icon: <Icon color="foreground" name="wallet" size="m" />,
  },
];
  ...
  const primaryContent = (
    <HStack justifyContent="center">
      <MultiContentModule
        ...
        {...exampleProps}
      >
        {listItems.map(({ icon, title, description }) => (
          <ListCell
            multiline
            description={description}
            media={<CellMedia name={icon} type="icon" />}
  ...
  const action = (
    <VStack spacingTop={1}>
      <ButtonGroup accessibilityLabel="Group" direction="vertical">
        <Button accessibilityLabel="continue" onPress={onPressConsole}>
          Continue
        </Button>
        <Button accessibilityLabel="cancel" onPress={onPressConsole} variant="secondary">
          Cancel
        </Button>
      </ButtonGroup>
    </VStack>
  ...
  const primaryContent = (
    <HStack justifyContent="center">
      <MultiContentModule bordered action={action} {...exampleProps}>
        <Box borderColor="primary" borderRadius="roundedMedium">
          <ListCell
...
export const WithEnd = () => {
  ...
  const end = (
    <VStack gap={1} spacingHorizontal={3}>
      <VStack>
        <HStack justifyContent="center" zIndex={1}>
          <Box background="background" spacingHorizontal={1.5}>
            <TextBody as="span" color="foregroundMuted">
              OR
            </TextBody>
          </Box>
        </HStack>
        <Divider offsetTop={1.5} spacingBottom={1.5} />
      </VStack>
      <HStack gap={2} justifyContent="space-between">
        {socialMediaItems.map(({ icon, name }) => (
          <VStack key={name} alignItems="center">
            ...
            >
              <Box spacing={2}>{icon}</Box>
            </Pressable>
            <TextLabel2 as="span" color="foregroundMuted" spacingTop={1}>
              {name}
            </TextLabel2>
          </VStack>
        ))}

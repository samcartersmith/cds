...
import type { IconName, ModalRenderChildren, NoopFn } from '@cbhq/cds-common';
...
export const WithButtonGroup = () => {
  const action = (closeModal: NoopFn) => (
    <VStack spacingTop={1}>
          ...
          <>
            ...
            <ModalBody contentContainerStyle={{ flexGrow: 1 }}>
              <MultiContentModule action={action(closeModal)} {...exampleProps}>
                <Box bordered borderColor="primary" borderRadius="roundedMedium">
                  <ListCell
      ...
      <VStack>
        <HStack justifyContent="center" zIndex={1}>
          <Box background="background" spacingHorizontal={1.5}>
            <TextBody color="foregroundMuted">OR</TextBody>
          </Box>
        </HStack>
      ...
      <HStack gap={2} justifyContent="space-between">
        {socialMediaItems.map(({ icon, name }) => (
          <VStack key={name} alignItems="center">
            ...
            <TextLabel2 color="foregroundMuted" spacingTop={1}>
              {name}
            </TextLabel2>
          </VStack>
        ))}

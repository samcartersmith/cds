...
const CustomSegmentedTab: TabComponent = (props) => (
  <SegmentedTab {...props} activeColor="warningForeground" color="primary" />
);
...
const AnotherCustomSegmentedTab: TabComponent = ({ id, label, disabled }) => {
  ...
  return (
    <Pressable
      ...
      onPress={handlePress}
    >
      <Box alignItems="center" spacingHorizontal={3} spacingVertical={1}>
        {typeof label === 'string' ? (
          <TextLabel2 color={isActive ? 'positive' : 'negative'} ellipsize="tail">
            {label}
          </TextLabel2>
        ) : (
          label

...
1. Add your `COMPONENT.mdx` or `COMPONENT.tsx` file to `eng/shared/design-system/website/src/pages`
...
1. Run `make prepare.illustrations`
2. Run both android and iOS and make sure the new illustrations didn't break either build.
3. Verify that the illustrations match what is on go/icon-illo-release-history
...
5. Go back to the `mono/repo` and cut a PR with the prepared illustrations.

## CDS Icon Release

1. Run `make prepare.icons`
2. Run both android and iOS and make sure the new illustrations didn't break either build.

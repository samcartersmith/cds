...
5. Navigate into the `temp-frontend-cds` repo and run the icon sync script. The script will create a new branch in the temp-oss-cds repo, sync the icons, then commit and push the changes
...
6. Open a PR in the [github.com/coinbase/cds](https://github.com/coinbase/cds) repo. Title the PR exactly the same as the commit message: `feat: Publish icons YYYY/MM/DD`. Take note of the PR number for the next step

7. Navigate into the `temp-oss-cds` repo and update the icons package changelog by completing the prompts as shown below
